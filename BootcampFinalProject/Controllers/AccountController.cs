...
namespace BootcampFinalProject.Controllers
{
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Login(LoginViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (user != null)
                {
                    ...
                    //var claims = new List<Claim> { new Claim(ClaimTypes.UserData, user.Image ?? "") };
        ...
        public async Task<IActionResult> Create(CreateViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    await _emailSender.SendEmailAsync(user.Email, "Hesap Onayı", $"Lütfen email hesabınızı onaylamak için linke <a href='http://localhost:5245{url}'>tıklayınız.</a>");
        ...
        public async Task<IActionResult> ForgotPassword(string Email)
        {
            ...
            await _emailSender.SendEmailAsync(Email, "Parola Sıfırlama", $"Lütfen email hesabınızı onaylamak için linke <a href='http://localhost:5245{url}'>tıklayınız.</a>");
